...
status = False#决定是否倒序遍历到药方

for fangji in reversed(all_list):
	fangji_name = fangji.find('a').text


	if fangji_name == "嗳气吞酸":#第一个症状，之前全是方剂
		status = True
		print("倒序遍历至“嗳气吞酸”，开始爬取内容")
		continue
	if status == False:
		continue
	...
	try:
		...
		data = {'方剂名称': fangji_name,
				'url': fangji_url,}
		temptitle = None#部分描述没有标题，用于将这样的描述添加到上一描述中
		for content in contentlist:
			if re.match(r, content.text) is not None:
				...
				description = re.sub(u"【.*?】", "", content.text)
				data[temptitle] = data[temptitle] + "    " + description#同一描述的两行用四个空格分隔
